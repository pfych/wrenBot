...
const fmEmbed = async function(msg) {
    let topAlbum = (await fm.userGetTopAlbums({user: "Puffycheeses", limit: limit, period: '7day'})).topalbums.album[Math.floor(Math.random() * (limit - 1))]
    ...
    console.log("sending")
    msg.channel.send("According to my last.fm habits this albums pretty good", { embed })
}
